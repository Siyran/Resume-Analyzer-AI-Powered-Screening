...
SMTP_PORT = 587
EMAIL_ADDRESS = os.environ.get("EMAIL_ADDRESS")   # Set this in your environment
...
def submit():
    try:
        ...
        if not name or not email or not phone:
            return jsonify({"result":"error","message":"Name, email, phone required"}),400
        if "resume" not in request.files:
            return jsonify({"result":"error","message":"Resume file required"}),400
        ...
        application_status = "Accepted" if score >= 85 else "Rejected"
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        sh = sheet_handle()
        sh.append_row([timestamp, name, email, phone, unique_name, score, application_status, reasons])
        ...
        if application_status == "Accepted":
            ...
            send_email(
                ...
                f"""
                <p>Hi {name},</p>
                <p>Great news! Based on your resume, you scored <b>{score}/100</b> and have been 
                <b>accepted</b> for the next stage.</p>
                <p>Please proceed here: <a href="{link}">Next Step</a></p>
                <p>We look forward to working with you.<br><br>Best,<br>HR Team</p>
                """
            )
        else:
            send_email(
                ...
                f"""
                <p>Hi {name},</p>
                <p>Thank you for applying. Based on our evaluation, your resume scored <b>{score}/100</b>. 
                Unfortunately, you were not selected at this time.</p>
        ...
        return jsonify({"result":"error","message":str(e), "trace": traceback.format_exc()}),500
